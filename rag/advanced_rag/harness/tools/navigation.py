...
def _entity_chunk_ids(entities: list[dict], wanted_names: list[str]) -> list[str]:
    """Return deduplicated source chunks for selected entity names or aliases."""
    wanted = {name.strip().lower() for name in wanted_names if isinstance(name, str) and name.strip()}
    chunk_ids: list[str] = []
    seen: set[str] = set()
    for entity in entities:
        names = {(entity.get("name") or "").strip().lower()}
        names.update((alias or "").strip().lower() for alias in (entity.get("aliases") or []))
        if not names & wanted:
            continue
    ...
    *,
    ...
    for kb_id, tenant_id, doc_ids in scopes:
        ...
        for hop in range(_KG_HOPS):
            ...
            seen_lower = {key.split(":", 1)[1] for key in ent_names if key.startswith(f"{kb_id}:")}
            neighbour_names = {name.strip() for relation in hop_relations for name in (relation["from"], relation["to"]) if name and name.strip()}
            unseen_names = {name for name in neighbour_names if name.lower() not in seen_lower}
            if not unseen_names:
                break
            neighbour_rows = await _kg_search(
                ...
                doc_ids,
                "entity",
                top_n=min(max(len(unseen_names), 1), _KG_NEIGHBORS),
                extra={"name_kwd": _endpoint_terms(unseen_names)},
                representation_kinds={"knowledge_graph"},
                scope_kwd=scope_kwd,
            )
            neighbours = [entity for entity in (_kg_parse_entity(row) for row in neighbour_rows.values()) if entity]
