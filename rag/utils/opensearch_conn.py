        ...
        for m in match_expressions:
            ...
            # Besides, Opensearch's DSL for KNN_search query syntax differs from that in Elasticsearch, I also made some adaptions for it
                ...
                if bool_inner.get("filter"):
                    knn_filter["filter"] = bool_inner["filter"]
                if bool_inner.get("must_not"):
                    knn_filter["must_not"] = bool_inner["must_not"]
                if knn_filter:
                    knn_query[vector_column_name]["filter"] = {"bool": knn_filter}
