...
LaneName = Literal["exact_phrase", "original_sparse", "original_dense", "expanded_sparse"]
...
_CANONICAL_LANES: tuple[LaneName, ...] = ("exact_phrase", "original_sparse", "original_dense", "expanded_sparse")
_SPARSE_LANES = frozenset({"exact_phrase", "original_sparse", "expanded_sparse"})
...
class FusionConfig:
    ...
    expanded_sparse_weight: float = 1.0
    reserve_per_lane: int = 1
    doc_level: bool = False
    diversity_enabled: bool = False
    diversity_lookahead: int = 3
    diversity_min_score_ratio: float = 0.8
    ...
    def from_env(cls, overrides: dict[str, Any] | None = None) -> "FusionConfig":
        ...
        return cls(
            ...
            exact_phrase_weight=_float_setting(overrides.get("fusion_exact_phrase_weight"), "FUSION_EXACT_PHRASE_WEIGHT", 1.0),
            expanded_sparse_weight=_float_setting(overrides.get("fusion_expanded_sparse_weight"), "FUSION_EXPANDED_SPARSE_WEIGHT", 1.0),
            reserve_per_lane=_nonnegative_int_setting(overrides.get("fusion_reserve_per_lane"), "FUSION_RESERVE_PER_LANE", 1),
            ...
            diversity_lookahead=_int_setting(overrides.get("fusion_diversity_lookahead"), "FUSION_DIVERSITY_LOOKAHEAD", 3),
            diversity_min_score_ratio=_float_setting(overrides.get("fusion_diversity_min_score_ratio"), "FUSION_DIVERSITY_MIN_SCORE_RATIO", 0.8),
        )

    def lane_weight(self, lane: str) -> float:
        lane_name = normalize_lane_name(lane)
        if lane_name == "exact_phrase":
            return self.exact_phrase_weight
        if lane_name == "original_dense":
            return self.dense_weight
        if lane_name == "expanded_sparse":
            return self.expanded_sparse_weight
    ...
    def fingerprint(self) -> str:
        return fusion_config_fingerprint(self)
...
class FusedCandidate:
    ...
    lane_contributions: dict[str, float] = field(default_factory=dict)
    ...
    original_index: int = 0
    provenance: dict[str, Any] = field(default_factory=dict)
    reserved_lanes: list[str] = field(default_factory=list)
    ...
    def to_trace_dict(self) -> dict[str, Any]:
        return {
            ...
            "lane_contributions": {k: _finite_or_zero(v) for k, v in sorted(self.lane_contributions.items())},
            "lineage": self.lineage[:_MAX_TRACE_LIST],
            "reserved_lanes": list(self.reserved_lanes),
            "rerank_rank": self.rerank_rank,
            "rerank_score": _safe_number(self.rerank_score),
...
class FusionResult:
    ...
    def to_kbinfos(self, original_kbinfos: dict[str, Any] | None = None, *, offset: int = 0, limit: int | None = None) -> dict[str, Any]:
        ...
        source_total = original_kbinfos.get("total")
        total = max(0, int(source_total)) if isinstance(source_total, (int, float)) and math.isfinite(source_total) else len(self.candidates)
        pagination_rejections = _candidate_rejections(
            self.candidates,
            selected,
            reason="pagination_truncation",
        )
        ...
        diagnostics["pagination"] = {
            ...
            "total": total,
            "truncated": bool(pagination_rejections),
            "rejection_lineage": pagination_rejections,
            "selected_lineage": [_final_lineage(candidate) for candidate in selected[:_MAX_TRACE_CANDIDATES]],
        }
        diagnostics["candidate_rejections"] = list(diagnostics.get("candidate_rejections") or [])
        diagnostics["candidate_rejections"].extend(pagination_rejections)
        diagnostics["candidate_rejections"] = diagnostics["candidate_rejections"][:_MAX_TRACE_CANDIDATES]
        diagnostics["final_lineage"] = [_final_lineage(candidate) for candidate in selected[:_MAX_TRACE_CANDIDATES]]
        out = dict(original_kbinfos)
        out["chunks"] = chunks
        out["doc_aggs"] = doc_aggs
        out["total"] = total
        out["diagnostics"] = dict(out.get("diagnostics") or {})
        out["diagnostics"]["fusion"] = diagnostics
        out["fusion"] = diagnostics
        return out
...
def rerank_fused_candidates(
    ...
    *,
    ...
    """Rerank an already lane-preserving pool, falling back to fused order."""
    bounded_max = max(0, int(max_candidates))
    pool = sorted(
        (deepcopy(candidate) for candidate in candidates),
        key=lambda candidate: (candidate.fused_rank, candidate.candidate_id),
    )[:bounded_max]
    query_hash = query_fingerprint(original_question)
    diagnostics: dict[str, Any] = {
        "attempted": bool(pool and rerank_mdl is not None),
        "query_hash": query_hash,
        "query_length": len(original_question or ""),
        "pool_size": len(pool),
    ...
    if not pool:
        ...
        return RerankSelectionResult(candidates=[], diagnostics=diagnostics)
    ...
    if rerank_mdl is None:
        ...
        try:
            docs = [_content_from_chunk(candidate.chunk) for candidate in pool]
            raw_scores, _ = rerank_mdl.similarity(original_question, docs)
            scores = [_finite_or_none(score) for score in list(raw_scores)]
            if len(scores) != len(pool) or any(score is None for score in scores):
                raise ValueError("reranker returned invalid scores")
            for candidate, score in zip(pool, scores, strict=True):
                candidate.rerank_score = cast(float, score)
            pool.sort(
        ...
        except Exception as exc:  # External providers are a deterministic fallback boundary.
            fallback_reason = f"reranker_error:{type(exc).__name__}"

    if fallback_reason:
        pool.sort(key=lambda candidate: (candidate.fused_rank, candidate.candidate_id))
        diagnostics["fallback_reason"] = fallback_reason
    for candidate in pool:
        _attach_rerank_metadata(candidate, query_hash=query_hash, fallback_reason=fallback_reason)
    diagnostics["candidates"] = [candidate.to_trace_dict() for candidate in pool[:_MAX_TRACE_CANDIDATES]]
    return RerankSelectionResult(candidates=pool, diagnostics=_sanitize_trace(diagnostics), fallback_reason=fallback_reason)


def normalize_lane_name(lane: Any) -> LaneName:
    value = str(lane or "").strip().lower()
    if value in {"exact_phrase", "phrase", "match_phrase"}:
        return "exact_phrase"
    if value in {"dense", "original_dense", "vector", "embedding", "semantic"}:
        return "original_dense"
    if value in {"expanded_sparse", "expanded", "expansion", "keyword"}:
        return "expanded_sparse"
    return "original_sparse"


def fusion_config_fingerprint(config: FusionConfig) -> str:
    encoded = json.dumps(config.to_trace_dict(), sort_keys=True, separators=(",", ":"), ensure_ascii=True)
    return hashlib.sha256(encoded.encode("utf-8")).hexdigest()


def resolve_request_lane_weights(
    config: FusionConfig,
    *,
    vector_weight: float,
    enabled_lanes: Iterable[str],
) -> FusionConfig:
    """Resolve one authoritative dense/sparse-family request budget."""
    ...
    return replace(
        config,
        exact_phrase_weight=sparse_weights.get("exact_phrase", 0.0),
        sparse_weight=sparse_weights.get("original_sparse", 0.0),
        dense_weight=bounded_vector_weight if dense_enabled else 0.0,
        expanded_sparse_weight=sparse_weights.get("expanded_sparse", 0.0),
    )
...
def current_weighted_fusion(candidates: list[RetrievalLaneCandidate], config: FusionConfig) -> list[FusedCandidate]:
    grouped = _group_candidates(candidates)
    fused = []
    for group in grouped.values():
        contributions = {
...
def fuse_retrieval_lanes(
    ...
    *,
    ...
    for lane, items in lanes.items():
        ...
        for candidate in lane_candidates:
            if candidate.fallback_note and candidate.fallback_note not in notes:
                notes.append(candidate.fallback_note)
    ...
    capacity = max(0, int(config.window if max_candidates is None else max_candidates))
    ranked_fused = fused
    fused = build_lane_preserving_pool(
        ranked_fused,
        max_candidates=capacity,
        reserve_per_lane=config.reserve_per_lane,
    )
    rejected = _candidate_rejections(ranked_fused, fused, reason="candidate_capacity")
    fusion_level = "chunk"
    if apply_controls:
        before_controls = list(fused)
    ...
    return FusionResult(candidates=fused, metrics=metrics, diagnostics=diagnostics, fallback_notes=notes)


def apply_post_rerank_controls(
    candidates: Iterable[FusedCandidate],
    config: FusionConfig,
    *,
    fallback_notes: list[str] | None = None,
) -> tuple[list[FusedCandidate], str, list[str]]:
    """Apply document and diversity controls after scoring, before pagination."""
    controlled = list(candidates)
    notes = list(fallback_notes or [])
    fusion_level = "chunk"

    if config.doc_level:
        if any(not candidate.doc_id for candidate in controlled):
            note = "doc_level_fallback:missing_doc_id"
            if note not in notes:
                notes.append(note)
        else:
            fusion_level = "doc"
            seen_docs: set[str] = set()
            document_selected: list[FusedCandidate] = []
            for candidate in controlled:
                doc_id = str(candidate.doc_id)
                if doc_id in seen_docs:
                    continue
                seen_docs.add(doc_id)
                document_selected.append(candidate)
            controlled = document_selected

    if config.diversity_enabled:
        controlled = _apply_relevance_preserving_diversity(
            controlled,
            lookahead=config.diversity_lookahead,
            min_score_ratio=config.diversity_min_score_ratio,
        )

    _assign_final_ranks(controlled)

    return controlled, fusion_level, notes


def build_lane_preserving_pool(
    fused: Iterable[FusedCandidate],
    *,
    max_candidates: int,
    reserve_per_lane: int = 1,
) -> list[FusedCandidate]:
    """Reserve a bounded prefix from each successful lane, then fill by fused rank."""
    if max_candidates <= 0:
        return []
    ordered = sorted(fused, key=lambda candidate: (candidate.fused_rank, candidate.candidate_id))
    oversubscribed = len(ordered) > max_candidates
    reserve_per_lane = max(0, int(reserve_per_lane))
    selected: list[FusedCandidate] = []
    selected_ids: set[str] = set()

    for lane in _CANONICAL_LANES:
        lane_prefix = sorted(
            (candidate for candidate in ordered if lane in candidate.lane_ranks),
            key=lambda candidate: (candidate.lane_ranks[lane], candidate.fused_rank, candidate.candidate_id),
        )[:reserve_per_lane]
        for candidate in lane_prefix:
            if candidate.candidate_id not in selected_ids and len(selected) < max_candidates:
                selected.append(candidate)
                selected_ids.add(candidate.candidate_id)
            if candidate.candidate_id in selected_ids and lane not in candidate.reserved_lanes:
                candidate.reserved_lanes.append(lane)

    if not oversubscribed:
        return _finalize_lane_preserving_pool(ordered)
    ...
    for candidate in ordered:
        ...
        selected.append(candidate)
        selected_ids.add(candidate.candidate_id)
    selected.sort(key=lambda candidate: (candidate.fused_rank, candidate.candidate_id))
    return _finalize_lane_preserving_pool(selected)


def _finalize_lane_preserving_pool(candidates: list[FusedCandidate]) -> list[FusedCandidate]:
    for candidate in candidates:
        candidate.reserved_lanes.sort(key=_CANONICAL_LANES.index)
        candidate.provenance["lane_reservations"] = list(candidate.reserved_lanes)
        metadata = candidate.chunk.get("_ragflow_fusion")
        if isinstance(metadata, dict):
            metadata["lane_reservations"] = list(candidate.reserved_lanes)
    return candidates
...
def compute_fusion_metrics(candidates: list[RetrievalLaneCandidate], fused: list[FusedCandidate] | None = None) -> dict[str, Any]:
    ...
    sparse = set().union(*(by_lane.get(lane, set()) for lane in _SPARSE_LANES))
    dense = by_lane.get("original_dense", set())
    union = sparse | dense
...
def make_fusion_trace_summary(
    *,
    ...
    capacity = config.window if max_candidates is None else max(0, int(max_candidates))
    summary = _sanitize_trace(
        {
            ...
            "exact_phrase_weight": config.exact_phrase_weight,
            "expanded_sparse_weight": config.expanded_sparse_weight,
            "config_fingerprint": config.fingerprint(),
            "doc_level": config.doc_level,
            "diversity_enabled": config.diversity_enabled,
            "diversity_lookahead": config.diversity_lookahead,
            "diversity_min_score_ratio": config.diversity_min_score_ratio,
            "lane_counts": dict(sorted(lane_counts.items())),
            "sparse_candidate_count": sum(lane_counts.get(lane, 0) for lane in _SPARSE_LANES),
            "dense_candidate_count": lane_counts.get("original_dense", 0),
    ...
    summary["candidate_rejections"] = list(candidate_rejections or [])[:_MAX_TRACE_CANDIDATES]
    summary["final_lineage"] = [_final_lineage(candidate) for candidate in fused[:_MAX_TRACE_CANDIDATES]]
    return summary
...
def attach_fusion_metadata(candidate: FusedCandidate, *, fallback_notes: list[str] | None = None) -> None:
    metadata = {
        ...
        "lane_contributions": {k: _finite_or_zero(v) for k, v in sorted(candidate.lane_contributions.items())},
        "lineage": candidate.lineage[:_MAX_TRACE_LIST],
        "lane_reservations": list(candidate.reserved_lanes),
...
def _make_fused_candidate(
    ...
    lineage = [
        {
            "lane": candidate.lane,
            "requested_lane": candidate.requested_lane,
            "effective_lane": candidate.effective_lane,
            "rank": candidate.rank,
            "raw_score": _finite_or_zero(candidate.raw_score),
            "normalized_score": _finite_or_zero(candidate.normalized_score),
            "contribution": _finite_or_zero((lane_contributions or {}).get(candidate.lane)),
        }
        for candidate in ordered
    ]
    return FusedCandidate(
        ...
        lane_contributions=dict(sorted((lane_contributions or {}).items())),
        lineage=lineage,
...
def _representative_candidate(group: list[RetrievalLaneCandidate]) -> RetrievalLaneCandidate:
    ...
    return sorted(group, key=lambda c: (c.rank, c.original_index, 0 if c.lane == "original_dense" else 1, c.candidate_id))[0]
...
def _apply_relevance_preserving_diversity(
    fused: list[FusedCandidate],
    *,
    lookahead: int,
    min_score_ratio: float,
) -> list[FusedCandidate]:
    """Locally promote unseen documents without globally round-robining ranks."""
    remaining = list(fused)
    if len(remaining) <= 2 or lookahead <= 1:
        return remaining
    selected: list[FusedCandidate] = []
    seen_docs: set[str] = set()
    while remaining:
        choice = 0
        first_doc = str(remaining[0].doc_id or remaining[0].candidate_id)
        if first_doc in seen_docs:
            for index, candidate in enumerate(remaining[1:lookahead], 1):
                candidate_doc = str(candidate.doc_id or candidate.candidate_id)
                baseline_score = _finite_or_none(remaining[0].rerank_score)
                candidate_score = _finite_or_none(candidate.rerank_score)
                if baseline_score is None:
                    baseline_score = _finite_or_zero(remaining[0].fused_score)
                if candidate_score is None:
                    candidate_score = _finite_or_zero(candidate.fused_score)
                relevance_floor = baseline_score * max(0.0, min(1.0, min_score_ratio))
                if candidate_doc not in seen_docs and candidate_score >= relevance_floor:
                    choice = index
                    break
        candidate = remaining.pop(choice)
        selected.append(candidate)
        seen_docs.add(str(candidate.doc_id or candidate.candidate_id))
    return selected


def _assign_final_ranks(candidates: list[FusedCandidate]) -> None:
...
def _candidate_rejections(
    before: Iterable[FusedCandidate],
    after: Iterable[FusedCandidate],
    *,
    reason: str,
) -> list[dict[str, Any]]:
    selected_ids = {candidate.candidate_id for candidate in after}
    return [
        {
            "candidate_id": candidate.candidate_id,
            "reason": reason,
...
def _final_lineage(candidate: FusedCandidate) -> dict[str, Any]:
    return {
        "candidate_id": candidate.candidate_id,
        "fused_rank": candidate.fused_rank,
        "rerank_rank": candidate.rerank_rank,
...
def _attach_rerank_metadata(candidate: FusedCandidate, *, query_hash: str, fallback_reason: str | None) -> None:
    metadata = candidate.chunk.setdefault("_ragflow_fusion", {})
    metadata.update(
        {
            "pre_rerank_rank": candidate.fused_rank,
            "pre_rerank_score": _finite_or_zero(candidate.fused_score),
            "rerank_rank": candidate.rerank_rank,
            "rerank_score": _safe_number(candidate.rerank_score),
...
def _nonnegative_int_setting(value: Any, env_name: str, default: int, *, legacy_env: str | None = None) -> int:
    value = _setting(value, env_name, default, legacy_env=legacy_env)
    try:
        parsed = int(value)
    except (TypeError, ValueError):
