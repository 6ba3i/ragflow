...
def _load_bot_api(monkeypatch, *, accessible, calls):
    ...
    async def _cross_languages_result(_tenant_id, _llm_id, query, _languages):
        return SimpleNamespace(query=query, status="disabled", identity=None)

    _stub(
        monkeypatch,
        "rag.prompts.generator",
        cross_languages=lambda *_a, **_k: None,
        cross_languages_result=_cross_languages_result,
        retrieval_keyword_expansion=lambda *_a, **_k: None,
        keyword_extraction=lambda *_a, **_k: None,
    )
